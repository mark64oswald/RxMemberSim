...
class ClinicalCriterion(BaseModel):
    ...
    lab_min_value: Optional[float] = None
    lab_max_value: Optional[float] = None
...
class ClinicalCriteriaSet(BaseModel):
    ...
    def evaluate(
        ...
        if self.require_all:
            overall_met = len(unmet) == 0
        else:
            overall_met = met_count > 0
    ...
    def _diagnosis_matches(self, code: str, required_codes: list[str]) -> bool:
        ...
        for required in required_codes:
            if code == required or code.startswith(required):
                return True
        return False
    ...
    def _check_specialist(
        self, criterion: ClinicalCriterion, prescriber_specialty: Optional[str]
    ) -> bool:
    ...
    def _check_age(self, criterion: ClinicalCriterion, member_age: int) -> bool:
        ...
        if criterion.min_age is not None and member_age < criterion.min_age:
            return False
        if criterion.max_age is not None and member_age > criterion.max_age:
            return False
        return True
    ...
    def _check_lab_result(
        ...
        if criterion.lab_min_value is not None and value < criterion.lab_min_value:
            return False
        if criterion.lab_max_value is not None and value > criterion.lab_max_value:
            return False
        return True
